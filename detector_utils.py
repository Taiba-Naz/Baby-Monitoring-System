...
def draw_box_on_image(num_babies_detect, score_thresh, scores, boxes, classes, im_width, im_height, image_np,
                      Line_Position2, Orientation):
    # Determined using a piece of paper of known length, code can be found in distance to camera
    focalLength = 875
    # The average width of a human BABY (inches) http://www.theaveragebody.com/average_hand_size.php
    ...
    color0 = (255, 0, 0)
    color1 = (0, 50, 255)
    for i in range(num_babies_detect):

        if (scores[i] > score_thresh):

            # no_of_times_babies_detected+=1
            # b=b+1
            # b=1
            # print(b)
            if classes[i] == 1:
            ...
            cv2.putText(image_np, 'baby ' + str(i) + ': ' + id, (int(left), int(top) - 5),
